...
abstract class BaseFragment(layoutId: Int) : Fragment(layoutId), Initialzation,
    ...
    private var interactor: OnFragmentInteractionListener? = null
    ...
    override fun <T : Fragment> startFragment(@IdRes frameId: Int, box: Box<T>): T {
        val fragment = box.cls.newInstance().apply {
            ...
            (this as? BaseFragment)?.setOnFragmentInteractionListener(this@BaseFragment)
        }
    ...
    private fun <T : Fragment> handleFlag(@IdRes frameId: Int, fragment: T, box: Box<T>): T {
        ...
        when (box.flag) {
            ...
            Box.FLAG_CLEAR_HISTORY -> {
                // Xoá lịch sử
                sfm.popBackStack(0, FragmentManager.POP_BACK_STACK_INCLUSIVE)
                // Thay thế fragment đầu tiên
                startNewFragment(frameId, fragment, box.getClassName(), false)
            }
        ...
        flagsUsed = box.flag
        return prefragment as? T ?: fragment
    }

    private fun <T : Fragment> startNewFragment(
        @IdRes frameId: Int,
        fragment: T,
        tag: String,
        isAdd: Boolean = true
    ...
    /**
     * Giử một thông điệp đến 1 Fragment đã đăng ký
     * @return Unit
     */
    ...
    fun setOnFragmentInteractionListener(interaction: OnFragmentInteractionListener) {
